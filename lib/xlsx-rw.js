...
exports.writeXlsxMultiPage = function(dataArray, nameArray, options, callback) {
  if (!options) {
    ...
    if (options.sheetName===undefined) {options.sheetName = makeDefault('sheetName');}
    if (options.fileName===undefined)  {options.fileName  = makeDefault('fileName');}
    if (options.path===undefined)      {options.path      = makeDefault('path');}
  }
  if (!callback) {
    callback = function(){};
  }
...
exports.parseXlsx = function(path, options, callback) {
  ...
  fs.readFile(path, function(err, rawData) {
    ...
    var data = parseXlsxRaw(rawData); // __dirname + '/myFile.xlsx'); // parses a file
    return callback(null, data);
  });
};
exports.parseXlsxRaw=function(rawData) {
