...
/**
 *
 * @param xlsxFile pathString or contentBuffer
 * @param mongoModel
 * @param options
 * @param callback
 */
exports.xlsx2MongoData = function(xlsxFile, mongoModel, options, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = undefined;
  }
  console.log(typeof(xlsxFile));
  if (typeof(xlsxFile)!='string'){
    var parsedRaw=xlsxRW.parseXlsxRaw(xlsxFile);
    var result=parseXlsx(parsedRaw, mongoModel);
    if (callback) callback(null,result.data, result.name);
    return result;
  }else
  xlsxRW.parseXlsx(xlsxFile, options, function(err, xlsxData) {
    if (err || !xlsxData) {
      return callback(err || new Error("No data"));
    }
    var result=parseXlsx(xlsxData, mongoModel);
    callback(null,result.data,result.name);
  });
};
function parseXlsx(xlsxData, mongoModel) {
    var data;
    var name;
